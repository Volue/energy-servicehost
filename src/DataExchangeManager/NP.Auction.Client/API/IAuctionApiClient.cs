namespace NP.Auction.Client.API
{
    ...
    public interface IAuctionApiClient
    {
        ...
        /// <param name="areaCodes">Collection of areas for additional filtering (optional). Provide null for no filtering</param>
        ...
        Task<IEnumerable<TradesSummary>> GetTradesAsync([Url] string auctionId,
            [Query(CollectionFormat.Multi)] string[] portfolios, [Query(CollectionFormat.Multi)] string[] areaCodes);
        ...
        Task<CurveOrder> PatchCurveOrderAsync([Url] Guid orderId, [Body] CurveOrderPatchRequest curveOrderPatchRequest);
        ...
        Task<BlockList> PatchBlockOrderAsync([Url] Guid orderId, [Body] BlockOrderPatchRequest blockOrderPatchRequest);
    }

    /// <summary>
    ///     Extending client for better error handling and support of cancelling blocks/curves and not yet published prices
    /// </summary>
    public static class AuctionApiClientExtensions
    {
        ...
        public static async Task<CurveOrder> ModifyCurveOrder(this IAuctionApiClient apiClient, Guid orderId,
            List<Curve> curves)
        {
            try
            {
                return await apiClient.PatchCurveOrderAsync(orderId, new CurveOrderPatchRequest {Curves = curves});
            }
            catch (ApiException exception)
            {
                throw ConstructApiException(exception);
        ...
        public static async Task<BlockList> CancelBlockOrder(this IAuctionApiClient apiClient, Guid orderId)
        {
            return await apiClient.ModifyBlockOrder(orderId, new List<Block>());
        }
        ...
        public static async Task<BlockList> ModifyBlockOrder(this IAuctionApiClient apiClient, Guid orderId, List<Block> blocks)
        {
            try
            {
                return await apiClient.PatchBlockOrderAsync(orderId, new BlockOrderPatchRequest { Blocks = blocks });
            }
            catch (ApiException exception)
            {
                throw ConstructApiException(exception);
        ...
        private static AuctionApiException ConstructApiException(ApiException exception)
        {
            ...
            {
                //var content = exception.GetContentAsAsync<Dictionary<string,string>>();
                //additionalErrorMessage = (string)content.Message;
                if (exception is ValidationApiException validationExc)
                {
                    additionalErrorMessage = validationExc.Content.Detail;
                }
            }
