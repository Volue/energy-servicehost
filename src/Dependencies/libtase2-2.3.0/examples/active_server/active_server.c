 ...
 *
 * This shows how to use a TASE.2 server with an active TCP connection (endpoint).
 * Can be used with passive_client example.
 *
 * Shows how to:
 *
...
main (int argc, char** argv)
{
    ...
    Tase2_ControlPoint vccCommand3 = Tase2_Domain_addControlPoint(vcc, "Command3", TASE2_CONTROL_TYPE_COMMAND, TASE2_DEVICE_CLASS_DIRECT, true, 1366);
    ...
    Tase2_BilateralTable blt2 = Tase2_BilateralTable_create("BLT_MZA_002_V1", icc2, "1.1.1.999", 12);
    ...
    Tase2_Endpoint_setRemoteIpAddress(endpoint, "localhost");
    Tase2_Endpoint_setLocalApTitle(endpoint, "1.1.1.997", 12);
    Tase2_Endpoint_setRemoteApTitle(endpoint, "1.1.1.998", 12);
    ...
    if (server) {
		...
		if (Tase2_Endpoint_waitForState(endpoint, TASE2_ENDPOINT_STATE_CONNECTED, 1500)) {
			...
			printf("ERROR: Failed to start endpoint. Reason can be that the binary/user requires root permissions"
