...
getFreeFrsm(MmsClient client)
{
    int i;

    MmsFileReadStateMachine* freeFrsm = NULL;

    /* TODO check if locking is required/possible */
    if (MmsClient_lockConnection(client)) {

        MmsEndpointConnection connection = client->connection;
        ...
        MmsClient_unlockConnection(client);
    }
...
static MmsFileReadStateMachine*
getFrsm(MmsClient client, int32_t frsmId)
{
    int i;
...
static int32_t
getNextFrsmId(MmsClient client)
{
    uint32_t nextFrsmId;
    ...
    uint32_t invokeId, ByteBuffer* response)
{
    ...
    if (hasFileName) {
        ...
        if (frsm != NULL) {
            FileHandle fileHandle = mmsMsg_openFile(MmsEndpoint_getFilestoreBasepath(connection->endpoint), filename, false);
    ...
    ByteBuffer* response)
{
    ...
    MmsFileReadStateMachine* frsm = getFrsm(client, frsmId);

    if (frsm != NULL) {
        mmsMsg_createFileReadResponse(client->connection->maxPduSize, invokeId, response, frsm);
    }
    else
