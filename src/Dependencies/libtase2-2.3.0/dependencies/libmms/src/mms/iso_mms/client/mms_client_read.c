...
mmsMsg_parseListOfAccessResults(MmsEndpointConnection connection, AccessResult_t** accessResultList, int listSize, bool createArray)
{
    ...
    for (i = 0; i < elementCount; i++) {
        ...
        if (presentType == AccessResult_PR_failure) {
            ...
            if (DEBUG_MMS_CLIENT)
                libmms_log(MMS_LOG_ERROR, connection, "MMS_CLIENT: unknown type %i", presentType);
        
            value = MmsValue_newDataAccessError(DATA_ACCESS_ERROR_TYPE_NOT_KNOWN_BY_RECEIVER);
        }
...
mmsClient_parseReadResponse(MmsEndpointConnection self, ByteBuffer* message, uint32_t* invokeId, bool createArray)
{
    ...
    if (rval.code != RC_OK) {
        ...
        if (DEBUG_MMS_CLIENT)
            libmms_log(MMS_LOG_ERROR, self, "MMS_CLIENT: read - invalid PDU");
    ...
    if (mmsPdu->present == MmsPdu_PR_confirmedResponsePdu) {
        ...
        if (mmsPdu->choice.confirmedResponsePdu.confirmedServiceResponse.present == ConfirmedServiceResponse_PR_read) {
            ...
            valueList = mmsMsg_parseListOfAccessResults(self, response->listOfAccessResult.list.array,
...
createVariableIdentifier(const char* domainId, const char* itemId)
{
    ...
    if (domainId) {
        ...
        variableIdentifier->variableSpecification.choice.name.present = ObjectName_PR_vmdspecific;
        variableIdentifier->variableSpecification.choice.name.choice.vmdspecific.buf = (uint8_t*) itemId;
        variableIdentifier->variableSpecification.choice.name.choice.vmdspecific.size = strlen(itemId);
    }
 ...
 */
int
mmsClient_createReadRequestMultipleValues(uint32_t invokeId, const char* domainId, LinkedList items, bool specificationWithResult,
        ByteBuffer* writeBuffer)
{
    MmsPdu_t* mmsPdu = mmsClient_createConfirmedRequestPdu(invokeId);
