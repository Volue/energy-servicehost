 ...
 *
 *  Copyright 2013-2021 MZ Automation GmbH
 *
 *  All rights reserved.
 */
		...
		int invokeId)
{
	...
	if (domainId != NULL) {
	    ...
	    if (domain == NULL) {
	        ...
	        if (DEBUG_MMS_SERVER)
	            libmms_log(MMS_LOG_ERROR, connection, "MMS_SERVER: get-var-access-attributes (invoke-id: %u) - domain %s not known --> service-error: object-non-existent", invokeId, domainId);
	...
	if (namedVariable == NULL) {
		if (DEBUG_MMS_SERVER)
		    libmms_log(MMS_LOG_ERROR, connection, "MMS_SERVER: get-var-access-attributes (invoke-id: %u) - variable %s not known --> service-error: object-non-existent", invokeId, nameId);
    ...
    if (response) {
        ...
        if (rval.encoded == -1) {
            ...
            if (DEBUG_MMS_SERVER)
                libmms_log(MMS_LOG_ERROR, connection, "MMS_SERVER: get-var-access-attributes (invoke-id: %u) - response does not fit in PDU --> service-error: other", invokeId);
	...
	if (rval.code == RC_OK) {
		if (request->present == GetVariableAccessAttributesRequest_PR_name) {
			if (request->choice.name.present == ObjectName_PR_domainspecific) {
                ...
                if (DEBUG_MMS_SERVER)
                    libmms_log(MMS_LOG_INFO, connection, "MMS_SERVER: get-var-access-attributes (invoke-id: %u) - domain specific %s:%s", invokeId, domainIdStr, nameIdStr);

				createVariableAccessAttributesResponse(connection, domainIdStr, nameIdStr, invokeId);
			    ...
			    if (DEBUG_MMS_SERVER)
			        libmms_log(MMS_LOG_INFO, connection, "MMS_SERVER: get-var-access-attributes (invoke-id: %u) - vmd specific %s", invokeId, nameIdStr);

			    createVariableAccessAttributesResponse(connection, NULL, nameIdStr, invokeId);
			    ...
			    if (DEBUG_MMS_SERVER)
			        libmms_log(MMS_LOG_ERROR, connection, "MMS_SERVER: get-var-access-attributes (invoke-id: %u) - object type not supported", invokeId);
            ...
            if (DEBUG_MMS_SERVER)
                libmms_log(MMS_LOG_ERROR, connection, "MMS_SERVER: get-var-access-attributes (invoke-id: %u) - address access not supported", invokeId);
        ...
        if (DEBUG_MMS_SERVER)
            libmms_log(MMS_LOG_ERROR, connection, "MMS_SERVER: get-var-access-attributes (invoke-id: %u) - invalid PDU", invokeId);
