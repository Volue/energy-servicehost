...
namespace tase2_server2
{
    class MainClass
    {
        ...
        /* callback handler to allow or reject client connections */
        private static bool connectionAcceptHandler(object parameter, Endpoint endpoint, string peerAddress, BilateralTable blt, AuthenticationMethod authMethod, byte[] authValue)
        {
            Console.WriteLine("Accept connection from " + peerAddress);
            Console.WriteLine("  authentication method: " + authMethod.ToString());
        ...
        private static void MyLogHandler(LogLevel level, LogSource source, Endpoint ep, EndpointConnection epCon, string message)
        {
            if (ep != null)
            {
                if (epCon != null) 
                    Console.WriteLine("LIBTASE2[{0}|{4}]: [{2}][{3}] {1}", level.ToString(), message, ep.Id, epCon.PeerAddress, source.ToString());
                else
                    Console.WriteLine("LIBTASE2[{0}|{3}]: [{2}] {1}", level.ToString(), message, ep.Id, source.ToString());
            }
            else
            {
                Console.WriteLine("LIBTASE2[{0}|{2}]: {1}", level.ToString(), message, source.ToString());
            }
        }
        ...
        public static void Main(string[] args)
        {
            ...
            Console.WriteLine("Using libtase2 version {0}", Tase2Library.GetVersionString());

            Console.WriteLine("Setting up TASE.2 server ...");

            Tase2Library.SetLogLevel(LogLevel.DEBUG);
            Tase2Library.SetLogHandler(MyLogHandler);
            ...
            if (args.Length > 0)
            {
                server.LocalIpAddress = args[0];
            }

            server.SetSupportedFeatures(SupportedFeatures.FB_1 | SupportedFeatures.FB_2 | SupportedFeatures.FB_4 | SupportedFeatures.FB_5);
            ...
            server.SetIdentity("MZA", "TASE.2", "2.1.0");
            ...
            server.Endpoint.SetConnectionAcceptHandler(connectionAcceptHandler, null);
            ...
            if (server.IsRunning())
            {
                ...
                while (running)
                {
                    ...
                    {	
                        ...
                        /* Send information messages to subscribers */
                        server.SendInformationMessage(null, 1, 1, msgId, "test message");
                        server.SendInformationMessage(icc1, 2, 2, msgId, "test message for ICC1");
